...
void Logo::paint(juce::Graphics &g) {
    ...
    gGlyphs.addLineOfText(logoFont, UILabels::Logo::appName, 0.0f, 0.0f);
    const float gWidth = gGlyphs.getBoundingBox(0, -1, false).getWidth();

    g.setFont(logoFont);
    g.setColour(juce::Colour(ColorPalette::primaryGreen));
    g.drawText(UILabels::Logo::appName, logoX, logoY, static_cast<int>(gWidth) + 2, logoH,
               juce::Justification::centredLeft);

    g.setColour(juce::Colour(ColorPalette::textBright));
    g.drawText(UILabels::Logo::appTitle, logoX + static_cast<int>(gWidth) + 1, logoY,
